...
let ``bounding_polygon_arcs_test`` () =
    for sweep in [true;false] do
        for large in [true;false] do
            let arc = Arc {Start=p 4.0 0.0;Radius=p 5.0 2.0;XAxisRotation=37.0<degree>;LargeArc=large;Sweep=sweep;End=p -1.0 3.0}
            check arc 0.0<parameter> 1.0<parameter>
            check arc 0.9<parameter> 0.15<parameter>
            check arc 0.5<parameter> 0.500001<parameter>
    let corrected = Arc {Start=p -4.0 0.0;Radius=p 1.0 1.0;XAxisRotation=0.0<degree>;LargeArc=false;Sweep=true;End=p 4.0 0.0}
...
let ``bounding_polygon_invalid_interval_and_arc_test`` () =
    ...
    let arc = Arc {Start=a;Radius=p 1.0 1.0;XAxisRotation=0.0<degree>;LargeArc=false;Sweep=true;End=a}
