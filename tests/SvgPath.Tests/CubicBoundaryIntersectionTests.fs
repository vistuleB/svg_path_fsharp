...
let ``cubic self intersections preserve closed endpoint parameters`` () =
    let hit = single (Bezier.CubicBezierData(p 0. 0.,p 0.1 1.,p -1. 0.2,p 0. 0.))
...
let ``cubic self intersections find start interior and reverse`` () =
    ...
    let hit = single (Bezier.CubicBezierData(a,b,c,d))
    Assert.Equal(0.0<parameter>,hit.S)
    near hit.T 0.5<parameter>
    let hit = single (Bezier.CubicBezierData(d,c,b,a))
...
let ``cubic self intersections reject one coordinate endpoint return`` () =
    Assert.Equal(Ok [],Bezier.cubicSelfIntersections (Bezier.CubicBezierData(p 0. 0.,p 1. 1.,p 0. 2.,p -3. 3.)))
[<Fact>]
