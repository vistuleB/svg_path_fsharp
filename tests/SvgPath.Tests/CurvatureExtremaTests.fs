...
let ``shifted stationary cubics keep both neighboring intervals`` () =
    ...
    for i in 1..98 do
        ...
        let actual = Curvature.segmentLeftNormalCuspParameters curve offset (options 1e-10 1) |> get
...
let ``touching cusp between sample points`` () =
    ...
    let roots = Curvature.segmentLeftNormalCuspParameters arch offset (options 1e-9 99) |> get
...
let ``two cusps in one old sample window`` () =
    ...
    let roots = Curvature.segmentLeftNormalCuspParameters arch (radius+0.000001<length>) (options 1e-10 99) |> get
...
let ``stationary cubic endpoint does not hide cusp`` () =
    ...
    let roots = Curvature.segmentLeftNormalCuspParameters curve (-125.0<length>/96.0) (options 1e-10 1) |> get
    near 1e-8 0.5<parameter> (Assert.Single roots)
[<Fact>]
let ``stationary interior is not a cusp root`` () =
    ...
    let roots = Curvature.segmentLeftNormalCuspParameters curve -1.0<length> (options 1e-10 1) |> get
    Assert.Equal(2,roots.Length)
...
let ``elliptical touch between sample points`` () =
    let roots = Curvature.segmentLeftNormalCuspParameters (arc (p 4. 2.) (p -4. 0.)) -8.0<length> (options 1e-9 99) |> get
    near 1e-8 0.5<parameter> (Assert.Single roots)
