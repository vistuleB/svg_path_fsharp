...
[<Fact>]
let ``ordinary segment directions are normalized`` () =
...
[<Fact>]
let ``collapsed cubic endpoint recovers the next nonzero derivative`` () =
...
[<Fact>]
let ``zero direction tolerance keeps the first nonzero candidate`` () =
...
[<Fact>]
let ``subpath directions use both sides of a corner`` () =
...
[<Fact>]
let ``subpath directions distinguish open ends and closed seam`` () =
    let a, b, c = point 0.0 0.0, point 1.0 0.0, point 1.0 1.0
...
[<Fact>]
let ``path directions delegate to the addressed subpath`` () =
