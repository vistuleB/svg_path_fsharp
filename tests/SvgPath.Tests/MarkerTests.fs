...
let private basicPose =
    ({ Kind = Marker.MarkerEnd
       ...
       Angle = 90.0<degree> }: Marker.MarkerPose)

let private basicLayout: Marker.MarkerLayout =
    { Reference = point 0.0 0.0
      MarkerWidth = 10.0<length>
      MarkerHeight = 10.0<length>
      MarkerUnits = Marker.UserSpaceOnUse
      StrokeWidth = 1.0<length>
      ViewBox = None
      PreserveAspectRatio = Marker.Meet Marker.XMidYMid }
...
let ``subpath poses returns start mid and end`` () =
    ...
    let poses = Marker.subpathPoses subpath Marker.Auto |> Result.defaultWith (failwithf "%A")
    Assert.Equal<Marker.MarkerKind list>([ Marker.MarkerStart; Marker.MarkerMid; Marker.MarkerEnd ], poses |> List.map _.Kind)
...
let ``closed subpath auto start reverse flips only start corner`` () =
    ...
    let poses = Marker.subpathPoses subpath Marker.AutoStartReverse |> Result.defaultWith (failwithf "%A")
...
let ``fixed orient uses one absolute angle`` () =
    let subpath = Subpath.polyline [ point 0.0 0.0; point 10.0 0.0; point 10.0 10.0 ] |> Result.defaultWith (failwithf "%A")
    let poses = Marker.subpathPoses subpath (Marker.MarkerOrient.Fixed 12.0<degree>) |> Result.defaultWith (failwithf "%A")
...
let ``auto orientation recovers collapsed cubic endpoint direction`` () =
    ...
    let poses = Marker.subpathPoses (Subpath.ofSegment segment) Marker.Auto |> Result.defaultWith (failwithf "%A")
    Assert.Equal(0.0<degree>, poses[0].Angle)
...
let ``auto orientation rejects fully collapsed subpath`` () =
    ...
    Assert.Equal(Error Marker.DegenerateMarkerTangent, Marker.subpathPoses (Subpath.ofSegment (Line(p, p))) Marker.Auto)
...
let ``path poses concatenates drawable subpaths and skips move only`` () =
    ...
    let poses = Marker.pathPoses path Marker.Auto |> Result.defaultWith (failwithf "%A")
    Assert.Equal(5, poses.Length)
    Assert.Equal<Marker.MarkerKind list>(
        [ Marker.MarkerStart; Marker.MarkerEnd; Marker.MarkerStart; Marker.MarkerMid; Marker.MarkerEnd ],
...
let ``layout transform stretches view box`` () =
    let layout =
        { basicLayout with
            ...
            Marker.PreserveAspectRatio = Marker.Stretch }
...
let ``layout transform meet aligns view box`` () =
    ...
    let layout = { basicLayout with Reference = point 5.0 5.0; MarkerWidth = 20.0<length>; ViewBox = Some { Min = point 0.0 0.0; Max = point 10.0 10.0 }; Marker.PreserveAspectRatio = Marker.Meet Marker.XMidYMid }
...
let ``layout transform slice aligns view box`` () =
    let pose = { basicPose with Point = point 100.0 100.0; Angle = 0.0<degree> }
    let layout = { basicLayout with Reference = point 5.0 5.0; MarkerWidth = 20.0<length>; ViewBox = Some { Min = point 0.0 0.0; Max = point 10.0 10.0 }; Marker.PreserveAspectRatio = Marker.Slice Marker.XMidYMid }
    let matrix = Marker.poseLayoutTransform pose layout |> Result.defaultWith (failwithf "%A")
    Assert.Equal(point 100.0 100.0, Affine.point matrix (point 5.0 5.0))
...
let ``layout transform scales stroke width units`` () =
    let layout =
        { basicLayout with
            ...
            Marker.MarkerUnits = Marker.StrokeWidth
            Marker.StrokeWidth = 4.0<length> }
    let matrix = Marker.poseLayoutTransform basicPose layout |> Result.defaultWith (failwithf "%A")
    Assert.Equal(basicPose.Point, Affine.point matrix (point 2.0 0.0))
...
let ``layout transform rejects invalid dimensions`` () =
    Assert.Equal(Error(Marker.InvalidMarkerWidth 0.0<length>), Marker.poseLayoutTransform basicPose { basicLayout with MarkerWidth = 0.0<length> })
    Assert.Equal(Error(Marker.InvalidMarkerHeight 0.0<length>), Marker.poseLayoutTransform basicPose { basicLayout with MarkerHeight = 0.0<length> })

[<Fact>]
let ``subpath poses rejects empty subpath`` () =
    Assert.Equal(Error Marker.EmptyMarkerSubpath, Marker.subpathPoses (Subpath.empty (point 0.0 0.0)) Marker.Auto)

[<Fact>]
let ``path poses returns empty for empty and move only paths`` () =
    Assert.Equal(Ok [], Marker.pathPoses Path.empty Marker.Auto)
    Assert.Equal(Ok [], Marker.pathPoses (Path.ofSubpaths [ Subpath.empty (point 1.0 2.0) ]) Marker.Auto)
